...
%Wilcokon signed rank (non-parametric) instead of paired t-test
% stats.median=median(v);
% if u>1    
%     topend=round(length(v)./u);
%     for ii=1:100
%     indices=randperm(length(v));
%     indices=indices(1:topend);
%     [ps(ii), h, t]=signrank(v(indices));
%     zs(ii)=t.zval;
%     end
%     stats.zWilcoxon=mean(zs);
%     stats.pWilcoxon = normcdf(stats.zWilcoxon,0,1);
%     stats.pWilcoxon2 =median(ps);
% else
%     [p, h, t]=signrank(v);
%     stats.zWilcoxon=t.zval;
%     stats.pWilcoxon = normcdf(t.zval,0,1);
% end
