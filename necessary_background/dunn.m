...
switch ctrl
    case 0 %Without control group
        kstar = 0.5*k*(k-1);
        old_alf = 0.05;
        new_alf = 1-(1 - old_alf).^(1/kstar); %Sidak's value of alpha
        vcrit = -realsqrt(2)*erfcinv(2-new_alf); %critical value; inverse standard normal probability distribution:norminv(p)=(−sqrt(2))×erfcinv(2p) SO norminv(1-0.5*alf)= (−sqrt(2))×erfcinv(2*(1-0.5 alpha)) = (−sqrt(2))×erfcinv(2-alpha))
...
% %                 end
    ...
    case 1 %With control group
        ...
        old_alf = 0.05;
        new_alf = 1-(1 - old_alf).^(1/(k-1)); %Sidak's value of alpha
        vcrit = -realsqrt(2)*erfcinv(2-new_alf); %critical value; inverse standard normal probability distribution:norminv(p)=(−sqrt(2))×erfcinv(2p) SO norminv(1-0.5*alf)= (−sqrt(2))×erfcinv(2*(1-0.5 alpha)) = (−sqrt(2))×erfcinv(2-alpha))
        pb{k-1,6} = []; I=1;
        for J=2:k
            count=count+1;
...
disp(cell2table(pb,'VariableNames',{'Comparison','Q_value','Crit_Q','alpha','corrected_alpha','Comment'}))
...
function Qvalue
    ...
    value_alfa = (1-normcdf(Q))*2;
    corr_alfa = 1 - nthroot(1-value_alfa,1/kstar);
    
    pb(count,1:5)={strcat(int2str(Mr(I,2)),'-',int2str(Mr(J,2))) Q vcrit value_alfa corr_alfa}; %vectors of comparisons
    if Q>vcrit
        pb(count,6)={'Reject H0'};
% %         comp=1; %more comparisons are required
    elseif Q<=vcrit
        pb(count,6)={'Fail to reject Ho'};
