...
timing_maps = 0; % set however you like TM = 1, VFM = 0
...
if exist(strcat(save_path, '/parameters_',maps,'.mat'), 'file') ~= 2 % Only needs to be done if you don't already have parameters_
    ve_data = monoTuned_load_timing_data_anonymous_incl_vfminfo(new_subjNames, paths, timing_maps,save_path);
end
if exist(strcat(save_path, '/time_series_',maps,'.mat'), 'file') ~= 2 % Only needs to be done if you don't already have time_series_
    time_series = monoTuned_load_time_series_anonymous(new_subjNames, paths, vfm_paths, timing_maps,save_path); % not used, but nice to have
end
...
medianPerSubj=0; % use 1, we take the medians per subject for model comparisons
minVE = 0;% use 0.2 for model comparisons
model_comparison = 1;
...
save_path = SAVE_PATH DATA; %TODO fill in
if exist(strcat(save_path, '/parameters_validation.mat'), 'file') ~= 2 % Only needs to be done if you don't already have test_models 
