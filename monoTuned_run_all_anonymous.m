...
vfm_paths{1}= PATH 1; %TODO fill in and adapt new_subjNames
vfm_paths{2}= PATH 2;
vfm_paths{3}= PATH 3;
vfm_paths{4}= PATH 4;
vfm_paths{5}= PATH 5;
...
if exist(strcat(save_path, '/ve_data_',maps,'.mat'), 'file') ~= 2 % Only needs to be done if you don't already have ve_data
...
if exist(strcat(save_path, '/time_series_',maps,'.mat'), 'file') ~= 2 % Only needs to be done if you don't already have time_series
    time_series = monoTuned_load_time_series_anonymous(new_subjNames, paths, vfm_paths, timing_maps,save_path); % never used, but nice to have
end
...
monoTuned_export_stats_anonymous(timing_maps, model_comparison, save_path)
    ...
    %% mesh comparisons (figure 2; suppl. figure 5) ==> needs mrVista 3; initHiddenGray; rmGet; open3DWindow; meshLoad; viewGet; viewSet; mrmSet; mrmMapVerticesToGray; meshGet; meshSet; meshRetrieveSettings; rmSelect; setDisplayMode; setClipMode; setCothresh; meshColorOverlay; rmLoadDefault; refreshScreen
...
monoTuned_mesh_comparisons_anonymous(new_subjNames, paths, save_path, minVE,minDuration,maxDuration,minPeriod,maxPeriod); % mrVista doesn't actually love this. If you get an error about  "datastructure"-something: run the mesh comparison file regularly (not as a function)
    ...
    %% data of separate voxels (figure 1; suppl. figure 1) ==> needs mrVista 3; initHiddenGray; intersectCols; rmRunDurFreq2d; rmGridFit; rfGaussian2d; pinv
...
fitting_figures_anonymous(paths, subj_id, whichVoxel, datapoint, roi_name, cv_run);
    ...
    %% model validation (suppl. figure 2) ==> check necessities
...
if exist(strcat(save_path, '/test_models.mat'), 'file') ~= 2 % Only needs to be done if you don't already have test_models 
    ...
    % training and crossvalidating the models will take a long time, run
    % this in parts
    whichModelMono = [13];
    combinedDTMono = 8:9;
    validation = 1;
    whichModelTuned = 4;
    combinedDTTuned = 10:17;
    CrossValidate2Component_anonymous(paths, whichModelMono, combinedDTMono, validation, whichModelTuned, combinedDTTuned)
end
